...
test.describe('Business Rules and Access Control (BR2, BR3, FR18, FR19)', () => {
  ...
  test('DISCREPANCY: Checking out with an empty cart is not blocked and can be completed with a $0.00 total', async ({ page, browserName }) => {
    ...
    // 1. Log in as standard_user with a guaranteed-empty cart, open the Cart page
  ...
  test('Logged-in session persists across checkout steps for a valid multi-step flow', async ({ page, browserName }) => {
    // Multi-step page transitions (login -> cart -> checkout-info -> overview -> complete) are prone to
    // webkit-only timeouts under full-suite parallel load; extend the timeout for webkit only.
    test.slow(browserName === 'webkit', 'Multi-step checkout flow is slow on webkit under parallel load');
  ...
  test('Attempting to check out again immediately after order completion (empty cart) reproduces the same empty-cart defect', async ({ page, browserName }) => {
    // Multi-step page transitions (full checkout flow, then a second checkout attempt) are prone to
    // webkit-only timeouts under full-suite parallel load; extend the timeout for webkit only.
    test.slow(browserName === 'webkit', 'Multi-step checkout flow is slow on webkit under parallel load');

    // 1. Complete a full order for one item so the cart becomes empty
    await page.goto('https://www.saucedemo.com');
    await page.locator('[data-test="username"]').fill('standard_user');
    await page.locator('[data-test="password"]').fill('secret_sauce');
    await page.locator('[data-test="login-button"]').click();
    ...
    // then click 'Checkout' again from the now-empty cart page
    await page.goto('https://www.saucedemo.com/cart.html');
    await page.locator('[data-test="checkout"]').click();

    // expect: The application again allows navigation to checkout-step-one.html despite zero items, reproducing the BR3/FR19 gap
    await expect(page).toHaveURL(/\/checkout-step-one\.html/);
